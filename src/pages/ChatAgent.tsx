...
// Define default settings for clearing
...
/** ________________________________________________
 * Project Panel Component (Memoized)
 * ________________________________________________ */
...
interface ProjectPanelProps {
  ...
  loading: boolean;
  fileInputRef: React.RefObject<HTMLButtonElement>;
  handleFileChange: (file: File | null) => void;
  handleExport: () => void;
  handleExportMarkdown: () => void;
  handleSystemPromptChange: (
    field: keyof SystemPromptConfig,
    value: string
  ) => void;
  handleSettingChange: (field: keyof Settings, value: string) => void;
  handleWorldBuildingChange: (
    field: keyof NonNullable<Settings["worldBuilding"]>,
    value: string
  ) => void;
  ...
  handleClearCharacters: () => void;
  handleClearChapters: () => void;
}
      ...
      <Card shadow="sm" p="md" pr="sm" radius="md" withBorder w="40%">
        ...
        <ScrollArea h="calc(100vh - 120px)" offsetScrollbars>
          ...
          <SettingsEditor
            ...
            handleClearSettings={handleClearSettings} // Pass down handler
            loading={loading} // Pass down loading state
          />
          <CharacterEditor
            characters={novel.characters}
            onChange={handleCharacterChange}
            handleClearCharacters={handleClearCharacters} // Pass down handler
            loading={loading} // Pass down loading state
          />
          <ChapterEditor
            ...
            handleClearChapters={handleClearChapters} // Pass down handler
            loading={loading} // Pass down loading state
          />
        </ScrollArea>
      </Card>
...
ProjectPanel.displayName = "ProjectPanel"; // Add display name for better debugging
...
export const ChatAgent: FC = () => {
  ...
  const handleClearSettings = useCallback(() => {
    if (
      window.confirm("本当に設定をクリアしますか？ この操作は元に戻せません。")
    ) {
      setNovel((prev) => ({
        ...prev,
  ...
  const handleClearCharacters = useCallback(() => {
    if (
      window.confirm(
        "本当に登場人物を全てクリアしますか？ この操作は元に戻せません。"
      )
    ) {
      setNovel((prev) => ({
        ...prev,
  ...
  const handleClearChapters = useCallback(() => {
    if (
      window.confirm(
        "本当に本文（章とシーン）を全てクリアしますか？ この操作は元に戻せません。"
      )
    ) {
      setNovel((prev) => ({
        ...prev,
        chapters: [], // Reset to empty array
      }));
    }
  }, []);
  ...
  return (
    <Flex direction="row" gap="md" p="md" w="100%" h="100vh" bg="dark.8">
      ...
      <ProjectPanel
        ...
        handleWorldBuildingChange={handleWorldBuildingChange}
        handleWritingStyleChange={handleWritingStyleChange}
        handleCharacterChange={handleCharacterChange}
        handleChapterChange={handleChapterChange}
        handleSceneChange={handleSceneChange}
        handleClearSettings={handleClearSettings}
        handleClearCharacters={handleClearCharacters}
        handleClearChapters={handleClearChapters}
      />
    </Flex>
  );
};
